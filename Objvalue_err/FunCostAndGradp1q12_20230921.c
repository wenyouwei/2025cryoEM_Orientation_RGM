        ...
        int nrhs, const mxArray *prhs[] ) 
{
    ...
    for (i=0; i<K; i++){
        ...
        for (j=0; j<K; j++){
            ...
            fagrad = sqrt(fa+1e-7); /*pow(fa,3/4); sqrt(fa);*/
            fagrad = pow(fagrad,3/2);
            z[0] += sqrt(sqrt(fa));;
            for (k=0; k<3; k++){                
                S[i6+k] = S[i6+k] + 0.5*a[k] * C[j2K+i2]/(fagrad+1e-20); 
                S[i6+k+3] = S[i6+k+3] + 0.5*a[k] * C[j2K+i2+1]/(fagrad+1e-20); 
            }
        }
    }
}
