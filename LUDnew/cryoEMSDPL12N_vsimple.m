...
Phi = W+G/mu; % provide an initial theta here:
% theta1 = C2theta(Phi,C,mu);
% S = Qtheta(theta1,C);%  S = Q(theta);
[S, theta]=Qtheta_vsimple(Phi,C,mu);
S=(S+S')/2;
% er_s = norm(SS-S);
% er_te = norm(theta1(:)-theta(:));
AS = ComputeAX(S);
resi = ComputeAX(G) - b;
...
for itr = 1:maxit
    ...
    Phi = W + ATy + G/mu;
%     theta1 = C2theta(Phi,C,mu);    
%     S = Qtheta(theta1,C); %then compute S = Q(theta)
