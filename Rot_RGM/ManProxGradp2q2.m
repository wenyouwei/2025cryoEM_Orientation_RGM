function est_rots =ManProxGradp2q2(C,Param,eta, beta)
...
[GradRi,FvalOld] = FunCostAndGradp2q2(Rold(:,1:2,:),C(1:2,:,:));
...
t   = 1;%0.99;    % 0.5 
tao  = 1.618; 
L = norm(GradRi(:)); % Lipschitz constant
L = 2;
...
for iter = 1: MaxIter
    ...
    for kk = 1:10 %10%8
        ...
        [GradRi,FvalNew] = FunCostAndGradp2q2(Rnew(:,1:2,:),C(1:2,:,:));
        % JRKnew = FvalOld - alpha*(norm(Rnew(:)-Rold(:))^2)/(2*mu);
        JRKnew =FvalOld - alpha*(norm(V(:))^2)/(2*eta);
        if FvalNew > JRKnew
            alpha = alpha*beta; 
        else
            break;
    ...
    %      fprintf('%3d, line search %2d: step length %1.2e, MSE: %f: Obj: %1.2e\n',iter,kk,alpha,MSEs22,FvalNew);
    %     end
    %      MSEs(iter) = check_MSE(est_rots, RefRot);
    %
